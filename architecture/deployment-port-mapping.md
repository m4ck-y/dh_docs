...
|----------|-----------|--------|--------|--------|
| `api_middleware` | `/` | **8080** | Frontend/Mobile only | Activo |

**Nota**: Este es el **único** servicio expuesto a Internet. Frontend web y móvil apuntan únicamente a `https://<domain>:8080` (o平衡ador). Todos los demás servicios corren en red privada.
...
| `dh_onboarding` | `/onboarding` | **8085** | `SERVICE_ONBOARDING_URL` | Activo |
...
| `dh_notify` (PulseCore) | `/notify` | **8091** | `SERVICE_NOTIFY_URL` | Testing |
| `dh_logger` (VitalTrace) | `/logger` | **8092** | `SERVICE_LOGGER_URL` | Testing |
...
app.mount("/notify", create_notify())      # → http://localhost:8091
app.mount("/logger", create_logger())  # → http://localhost:8092
```
...
- [ ] **VPS Firewall**: Solo puerto 8080 abierto al público
...
- [ ] **Load Balancer** (futuro):平衡ador apunta solo a `api_middleware:8080` en multiples réplicas
...
| `api_middleware` | 8000 | **8080** | Cambiar a 8080 |

**Motivo**: Uniformizar rango 8081-8092 para todos los microservicios backend. El puerto 8080 reservado exclusivamente para el gateway.
