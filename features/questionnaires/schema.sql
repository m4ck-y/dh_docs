...
COMMENT ON COLUMN question.list_options IS 'Opciones de la pregunta (tipos SINGLE_CHOICE/MULTIPLE_CHOICE) como UNION taggeada JSONB: {"source":"static","items":[{uuid,value,label,description?,order?,url?},...]} o {"source":"catalog","catalog":{"key":"<key del registro>"}}. uuid es un identificador global y estable por opcion (usado en PATCH/DELETE /questions/options/{uuid_option}). value es number (escalas/scoring) o string. Es NULL cuando la pregunta no es de eleccion. Obligacion (app/Pydantic): en choice, source="static" exige items no vacio; source="catalog" exige key existente en el registro (ver ADR 046). Los sentinels ("Ninguna", "Prefiere no decirlo") son items del catalogo con su propio value; el dominio los mapea a NULL (o 0/otro segun el campo).';
