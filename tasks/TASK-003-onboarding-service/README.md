...
Servicio `dh_onboarding` que orquesta el flujo de registro de nuevos usuarios (Design B — Person-first en personal-info). Acceso directo a PostgreSQL via SQLAlchemy async.
...
- [x] Modelos SQLAlchemy para schemas `people`, `auth`, `expedient` (referenciando a `dh_core`, `dh_auth` y `dh_storage`).

  - [x] `POST /v1/onboarding/{id}/documents` — sube documento a `dh_storage` (schema expedient).

  - **2026-04-26**: Renombramiento global de servicios orquestados (dh_core, dh_auth, dh_expedient → dh_storage).
