...
- [x] Convertir a JSON los **8 instrumentos** de `docs/diagrams/{1,2,3}`:
  - mental: `hads`, `cdi`, `gds`, `phq`, `gad`, `pss`
  - social: `crafft`
  - físico: `ipaq`
  - Fuentes: **metadata** `docs/diagrams/catalog/instruments.csv` +
    `flows/<key>.mmd` (preguntas/opciones) + `reviews/<key>-review.md`
    (interpretación) + `expressions/` (scoring/evaluation).
- [x] `bank/instruments/<key>.json` = **definición canónica** (misma forma que el
      documento MongoDB —`catalog/CLASS.mmd`— y que la respuesta del endpoint del
      catálogo; un solo artefacto).
- [x] `bank/instruments/<key>.md` = lectura para **usuario final** (formato legacy
      `PHQ9.md`: metadata + descripción amigable + preguntas/opciones).
- [x] Asignar `list_categories[]` a cada instrumento (usar `bank/categories.json`).
